...
def mapping_stats_fractional(assign_df: pd.DataFrame, total_reads_sidecar: int) -> Dict[str, Any]:
    ...
    # Row sums of probabilities, defensive clipping and NaN handling
...
def main():
    ...
    ap.add_argument("--runs-dir", required=True, help="Path to results/emu_runs_* for a batch")
    ap.add_argument("--outdir", required=True, help="Where to write tables (e.g., results/tables_*)")
    ...
    ap.add_argument("--skip-assign", action="store_true",
                    help="Skip loading read-assignment distributions (faster; mapping_stats use totals only).")
    args = ap.parse_args()
